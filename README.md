...
PC takes a screenshot using mss, converts the mean color to a single RGB value using NumPy, sends them over to my Raspberry Pi Pico 2 W via Wi-Fi, The Pi calculates which of the 20 colors my LED strip has is the closest (or OFF if color is close to black and also factors in brightness), and blasts the corresponding IR frequencies to my LED strip.
